...
if __name__ == '__main__':
    ...
    parser.add_argument("--control_path", help="Path to control columns.")
    ...
    #### Set up control dataframe
    with open(args.control_path, 'r') as f:
        control_columns = [ x.strip('\n') for x in f ]
    control_annot_df  = gene_annot_df.copy()

    #### Sort for canonical ordering
    all_feature_files = sorted([f for f in glob.glob(feature_dir + "/*.txt")])

    all_mat_data = []
    all_col_data = []
    curr_block_index = 1 # start all non-control features at 1
    for f in all_feature_files:
        f_df = pd.read_csv(f, sep="\t", index_col=0).astype(np.float64)

        # Check for controls
        control_annot_df = control_annot_df.merge(
            f_df[[ x for x in f_df.columns if x in control_columns ]],
            how="left",
            left_index=True,
            right_index=True
        )
        ...
        f_df = gene_annot_df.merge(
            f_df[[ x for x in f_df.columns if x not in control_columns ]],
            how="left",
            left_index=True,
            right_index=True
        )
    ...
    # Now save controls
    control_mat = control_annot_df.values
    control_cols = control_annot_df.columns.values
    np.save(save_prefix + ".mat.0.npy", control_mat)
    np.savetxt(save_prefix + ".cols.0.txt", control_cols, fmt="%s")
